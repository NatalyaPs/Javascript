...
class Order {
    ...
    getTotalPrice() {
        const prices = this.products.map((product) => Number(product.price));
        const sum = prices.reduce((acc, num) => acc + num);
        return sum;
    }
}
...
class Student {
    ...
    // или объявить свойства без конструктора? :
    #name = '';
    #age = 0;
    #averageGrade = 0;

    // но методы не приватные - ???
    setName(name) {
        this.#name = name;
    }
    ...
    displayInfo() {
        console.log(`Студен ${this.#name}, возраст ${this.#age}, средний балл: ${this.#averageGrade}`);
    }
}
//  ВОЗМОЖНО, Я ЧТО-ТО НЕДОПОНЯЛА  ))
const student = new Student();
student.setName('Питер Паркер');
student.setAge(20);
student.setAverageGrade(85);
...
// пробовала разобраться:
console.log(student.name); // undefined
// student.#name(); // недоступное свойство

class NewStudent extends Student {
    // свойтва #name, #age и #averageGrade недоступны вне класса Student, т.к оно имеет закрытый идентификатор
    displayInfoNewStudent() {
        // console.log(`Студен ${this.#name}, возраст ${this.#age}, средний балл: ${this.#averageGrade}`);
    }
}